...
@Component({
    selector: 'app-customer-date-wise-rent-report',
    templateUrl: './customer-date-wise-rent-report.component.html',
    styleUrls: ['./customer-date-wise-rent-report.component.scss']
})
export class CustomerDateWiseRentReportComponent implements OnInit {
    currency: any;
    selectedDevice;
    loginData: any;
    tableData: any;
    ...
    onSubmit() {
       if(this.typeOne.valid){
        ...
        this.AdminService.GetCustomerRentReport(this.sendObj).subscribe(res => {
            // console.log(res)
            this.formServiceData.listTotal = []
            this.formServiceData.chartOptions = []
            if (res.statusCode == 200) {
                if (res.result < 1) {
                    ...
                    if (this.loginData.userRole != 1) {
                        if (this.tableData.length > 0 && this.tableData[0].rentAgentlist.length > 0) {
    ...
    agentFilter(agent: any) {
        ...
        if (agent == 0 || agent == "0") {
            ...
            var agentSearched = this.bkUpTableList2[0].rentAgentlist.filter(x => x.customerId == agent);
    ...
    getTotal(index1: number, index2: number, data, customer_name: string, type: string) {
        ...
        let newData = data
        if (tempIndex == -1) {
            var amount = 0, discount = 0, coupon = 0, lateChages = 0, price = 0, quantity = 0
